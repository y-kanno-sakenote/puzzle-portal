...
def build():
    ...
    # 段階割り当て: 一次キー=探索空間 S（5x5 は 10x10 より必ず小さい）、二次キー=D
    small = sorted([i for i in items if i["w"] == 5], key=lambda i: i["metrics"]["D"])
    big = sorted([i for i in items if i["w"] == 10], key=lambda i: i["metrics"]["D"])
    for i in small:
        i["tier"] = "入門"
