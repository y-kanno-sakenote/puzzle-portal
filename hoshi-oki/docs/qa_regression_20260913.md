...
3. 難度実態（自前簡易論理ソルバー）: 入門10/10・初級10/10が論理のみで解け、
   中級0/10（申告nodes 2〜12）・上級0/10（申告nodes 20〜62）はバックトラックが必要 → FAILではなく設計通り
```
...
全問 `1`。FAIL 0件。最遅個体 index=23 1ms。

## 4. localStorage 壊れ値
保存キー名: `hoshi-oki`
2本の`<script>`を連結してvm実行、9パターン全部 OK（未捕捉例外なし）。
※検証ハーネスのDOMスタブに`style.setProperty`が無いと`elBoard.style.setProperty is not a function`で
落ちる（ゲーム側の欠陥ではなくスタブ不備）。スタブ補完後は全パターン通過。
`loadSave()`が`Number.isInteger`で小数・型不正を弾く。

## 5. docs/spec.md との照合
nodes閾値表（中級1〜12・上級≥20、実物レンジ2〜12/20〜62）に対し、実データの中級nodes範囲=(2,12)・
上級nodes範囲=(20,62)で一致。乖離なし。
